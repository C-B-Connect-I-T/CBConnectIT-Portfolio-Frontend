package cbconnectit.portfolio.web.pages.companies.manage
...
@Page("/admin/companies/manage")
@Composable
fun AdminManageCompaniesPage() = authenticatedGuard {
    val ctx = rememberPageContext()
    val companyId = ctx.route.params["id"]
    val viewModel = rememberViewModel(cached = false) { ManageCompaniesViewModel(companyId = companyId) }
    val state by viewModel.state.collectAsState()
    val isEdit = companyId != null

    LaunchedEffect(Unit) {
        viewModel.effect.collectLatest { effect ->
            when (effect) {
                ManageCompaniesContract.Effect.NavigateBackToCompanies ->
                    ctx.router.navigateTo(Navigation.Screen.Admin.Companies.Index.route)
            }
        }
    }

    LaunchedEffect(Unit) {
        viewModel.sendIntent(ManageCompaniesContract.Intent.LoadInitialData)
    }

    AdminPageLayout(
        title = if (isEdit) "Edit company" else "Create company"
    ) {
        ManageCompaniesPageContent(
            state = state,
            isEdit = isEdit,
...
@Composable
private fun ManageCompaniesPageContent(
    state: ManageCompaniesContract.State,
    isEdit: Boolean,
    sendIntent: (ManageCompaniesContract.Intent) -> Unit
) {
    ...
    Column(
        ...
        TitleLarge(
            text = if (isEdit) "Edit company" else "Create company",
        ...
        DsEditableField(
            ...
            onValueChange = { sendIntent(ManageCompaniesContract.Intent.UpdateName(it)) },
            required = true,
        ...
        DsEditableField(
            modifier = Modifier.fillMaxWidth(),
            id = "company-links-input",
            label = "Links",
            placeholder = "https://company.com, https://linkedin.com/company/example",
            value = state.linksInput,
            onValueChange = { sendIntent(ManageCompaniesContract.Intent.UpdateLinksInput(it)) },
            backgroundColor = colorScheme.surfaceContainer,
            focusBorderColor = colorScheme.primary.toRgb().copyf(alpha = 0.6f)
        )
        ...
        Row(
            ...
            if (isEdit) {
                DestructiveOutlinedButton(
                    ...
                    onClick = { sendIntent(ManageCompaniesContract.Intent.ShowDeleteDialog) }
                ) {
                    Text("Delete")
                }
            } else {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(12.px), verticalAlignment = Alignment.CenterVertically) {
                OutlinedButton(
                    borderRadius = DsBorderRadius(8.px),
                    onClick = { sendIntent(ManageCompaniesContract.Intent.Cancel) }
                ) {
                    Text("Cancel")
                }
                ...
                FilledButton(
                    borderRadius = DsBorderRadius(8.px),
                    onClick = { sendIntent(ManageCompaniesContract.Intent.SaveCompany) }
                ) {
    ...
    if (state.showDeleteDialog) {
        Dialog(onHideDialog = { sendIntent(ManageCompaniesContract.Intent.HideDeleteDialog) }) {
            Column(
                modifier = Modifier.fillMaxWidth(),
                ...
                Row(
                    ...
                    OutlinedButton(
                        borderRadius = DsBorderRadius(8.px),
                        onClick = { sendIntent(ManageCompaniesContract.Intent.HideDeleteDialog) }
                    ) {
                        Text("Cancel")
                    }
                    ...
                    DestructiveFilledButton(
                        borderRadius = DsBorderRadius(8.px),
                        onClick = { sendIntent(ManageCompaniesContract.Intent.ConfirmDelete) }
                    ) {
